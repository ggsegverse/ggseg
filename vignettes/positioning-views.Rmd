...
atlases. `position_brain_polygon()` controls how those views are arranged in
the final plot.
...
ggplot() +
  geom_brain_polygon(
    atlas = dk(),
    position = position_brain_polygon(view ~ hemi),
    show.legend = FALSE
  ) +
  theme_void()
```
...
ggplot() +
  geom_brain_polygon(
    atlas = aseg(),
    position = position_brain_polygon("horizontal"),
    show.legend = FALSE
  ) +
  theme_void()
```
...
ggplot() +
  geom_brain_polygon(
    atlas = aseg(),
    position = position_brain_polygon(ncol = 3),
    show.legend = FALSE
  ) +
  theme_void()
```
...
ggplot() +
  geom_brain_polygon(
    atlas = aseg(),
    position = position_brain_polygon(type ~ .),
    show.legend = FALSE
  ) +
  theme_void()
```
...
Use `annotate_brain_polygon()` to label each view with its name. For cortical
...
Store the `position_brain_polygon()` specification in an object so both layers
...
ggplot() +
  ...
  annotate_brain_polygon(atlas = dk(), position = pos) +
  theme_void()
```
...
pos <- position_brain_polygon(nrow = 2)
ggplot() +
  geom_brain_polygon(atlas = aseg(), position = pos, show.legend = FALSE) +
  annotate_brain_polygon(atlas = aseg(), position = pos) +
  theme_void()
```
...
ggplot() +
  geom_brain_polygon(
    atlas = aseg(),
    position = position_brain_polygon(zoom = focus, zoom_pad = 0.01),
    show.legend = FALSE
  ) +
  theme_void()
```
...
ggplot() +
  geom_brain_polygon(
    atlas = aseg(),
    position = position_brain_polygon(zoom = focus, zoom_pad = 0.25),
    show.legend = FALSE
  ) +
  theme_void()
```
