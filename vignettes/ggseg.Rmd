...
`geom_brain_polygon()` works like any ggplot2 geom. Pass it an atlas and it
draws the regions:

```{r}
#| label: fig-basic-brain
#| fig-cap: "Default brain plot using geom_brain_polygon() with the dk atlas."
ggplot() +
...
`position_brain_polygon()` controls how they're laid out, using formula
syntax borrowed from `facet_grid()`:

```{r}
#| label: fig-position-formula
#| fig-cap: "Brain views arranged using formula syntax in position_brain_polygon()."
ggplot() +
  geom_brain_polygon(
    atlas = dk(),
...
ggplot() +
  geom_brain_polygon(
    atlas = dk(),
    position = position_brain_polygon(c(
      "right lateral",
      "right medial",
      "left lateral",
      "left medial"
...
ggplot() +
  geom_brain_polygon(atlas = dk(), view = "lateral", show.legend = FALSE)
```
...
ggplot() +
  geom_brain_polygon(atlas = dk(), hemi = "left")
```
...
(typically `region` or `label`). Pass it to `geom_brain_polygon()` through
...
variable and `geom_brain_polygon()` replicates the full atlas in each panel:
...
ggplot() +
  geom_brain_polygon(atlas = dk(), data = data, aes(fill = reg_col)) +
  scale_fill_brain_manual(palette)
```

## Scales and themes

Atlas colour palettes are applied automatically by `geom_brain_polygon()`.
