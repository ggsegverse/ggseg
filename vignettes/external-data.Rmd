...
`geom_brain_polygon()` joins your data to the atlas by `region` (and by
...
Pass the data to `geom_brain_polygon()` through its `data` argument and map
...
ggplot() +
  geom_brain_polygon(atlas = dk(), data = some_data, mapping = aes(fill = p))
```
...
`facet_grid()` work as you'd expect. `geom_brain_polygon()` replicates the
